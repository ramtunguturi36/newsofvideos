...
router.post("/verify-payment", authMiddleware, async (req, res) => {
  try {
      ...
      items.map(async (item) => {
        ...
        if (item.type === "template") {
          ...
          try {
            ...
            if (template) {
              console.log(
                `✅ Video template found: ID=${template._id}, title=${template.title}`,
              );
              return {
                ...item,
                templateId: item.id,
          ...
          try {
            ...
            if (folder) {
              console.log(
                `Folder found: ID=${folder._id}, name=${folder.name}`,
              );
              return {
                ...item,
                folderId: item.id,
          ...
          try {
            ...
            const PictureTemplate = (
              await import("../models/PictureTemplate.js")
            ).default;
            const template = await PictureTemplate.findById(item.id);
            if (template) {
              console.log(
                `✅ Picture template found: ID=${template._id}, title=${template.title}`,
              );
              return {
                ...item,
                templateId: item.id,
          ...
          try {
            ...
            const PictureFolder = (await import("../models/PictureFolder.js"))
              .default;
            const folder = await PictureFolder.findById(item.id);
            if (folder) {
              console.log(
                `Picture folder found: ID=${folder._id}, name=${folder.name}`,
              );
              return {
                ...item,
                folderId: item.id,
          ...
          try {
            // Fetch the actual video content to get the real data
            const VideoContent = (await import("../models/VideoContent.js"))
              .default;
            const video = await VideoContent.findById(item.id);
            if (video) {
              console.log(
                `✅ Video content found: ID=${video._id}, title=${video.title}`,
              );
              return {
                ...item,
                templateId: item.id,
              ...
              console.log("❌ Video content not found for ID:", item.id);
            }
          } catch (error) {
          ...
          try {
            // Fetch the actual video folder to get the real data
            const VideoFolder = (await import("../models/VideoFolder.js"))
              .default;
            const folder = await VideoFolder.findById(item.id);
            if (folder) {
              console.log(
                `Video folder found: ID=${folder._id}, name=${folder.name}`,
              );
              return {
                ...item,
                folderId: item.id,
          ...
          try {
            // Fetch the actual audio content to get the real data
            const AudioContent = (await import("../models/AudioContent.js"))
              .default;
            const audio = await AudioContent.findById(item.id);
            if (audio) {
              console.log(
                `✅ Audio content found: ID=${audio._id}, title=${audio.title}`,
              );
              return {
                ...item,
                templateId: item.id,
              ...
              console.log("❌ Audio content not found for ID:", item.id);
            }
          } catch (error) {
          ...
          try {
            // Fetch the actual audio folder to get the real data
            const AudioFolder = (await import("../models/AudioFolder.js"))
              .default;
            const folder = await AudioFolder.findById(item.id);
            if (folder) {
              console.log(
                `Audio folder found: ID=${folder._id}, name=${folder.name}`,
              );
              return {
                ...item,
                folderId: item.id,
            ...
            console.error("Error fetching audio folder:", error);
          }
        }
        return item;
      }),
    ...
    console.log(
      "✅ Purchase items with template IDs:",
    ...
    try {
      for (const item of itemsWithQR) {
        ...
        if (item.type === "template" && item.templateId) {
          console.log(
            "🔑 Granting video template access for:",
            item.templateId,
          );
          ...
          console.log(
            "🔑 Granting picture template access for:",
            item.templateId,
          );
          await grantTemplateAccess(
            req.user.userId,
        ...
        } else if (item.type === "video-content" && item.templateId) {
          console.log("🔑 Granting video content access for:", item.templateId);
          await grantTemplateAccess(
            req.user.userId,
            item.templateId,
            purchase._id,
          );
          console.log("✅ Video content access granted");
        } else if (item.type === "video-folder" && item.folderId) {
          console.log("🔑 Granting video folder access for:", item.folderId);
          await grantFolderAccess(req.user.userId, item.folderId, purchase._id);
          console.log("✅ Video folder access granted");
        } else if (item.type === "audio-content" && item.templateId) {
          console.log("🔑 Granting audio content access for:", item.templateId);
          await grantTemplateAccess(
            req.user.userId,
            item.templateId,
            purchase._id,
          );
          console.log("✅ Audio content access granted");
        } else if (item.type === "audio-folder" && item.folderId) {
          console.log("🔑 Granting audio folder access for:", item.folderId);
          await grantFolderAccess(req.user.userId, item.folderId, purchase._id);
          console.log("✅ Audio folder access granted");
        } else {
