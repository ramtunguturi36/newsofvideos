...
router.post(
  ...
  async (req, res) => {
    try {
      const { title, description, basePrice, discountPrice, parentId } =
        req.body;
      if (!title || !basePrice)
...
router.get("/hierarchy", async (req, res) => {
  try {
    ...
    const [folders, rawTemplates, path] = await Promise.all([
      ...
      Template.find(folderId ? { folderId } : { folderId: null }).sort({
        createdAt: -1,
      }),
      buildPath(folderId),
    ]);
...
router.put(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const { title, description, basePrice, discountPrice, folderId } =
        req.body;
