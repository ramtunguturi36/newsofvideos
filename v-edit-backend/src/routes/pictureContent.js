...
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 1000 * 1000 * 50 } }) // 50MB limit for images

const router = express.Router()

// Create picture folder
router.post('/picture-folders', authMiddleware, roleMiddleware(['admin']), async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!previewFile || !downloadFile) return res.status(400).json({ message: 'Files missing' })
      ...
      const watermarkedPreview = await addWatermark(previewFile.buffer, 'PREVIEW ONLY - DO NOT COPY');
      
      const [previewUrl, downloadUrl] = await Promise.all([
        uploadToR2({ bucket, key: previewKey, contentType: previewFile.mimetype, body: watermarkedPreview }),
        uploadToR2({ bucket, key: downloadKey, contentType: downloadFile.mimetype, body: downloadFile.buffer }),
      ])
...
router.get('/test-picture-templates', async (req, res) => {
  try {
    ...
    console.log('🧪 Test endpoint - All picture templates:', allTemplates.length)
    console.log('🧪 Template IDs:', allTemplates.map(t => t._id))
    console.log('🧪 Template details:', allTemplates.map(t => ({ id: t._id, title: t.title, folderId: t.folderId })))
...
router.get('/picture-hierarchy', async (req, res) => {
  try {
    ...
    const [folders, rawTemplates, path] = await Promise.all([
      ...
      PictureTemplate.find(folderId ? { folderId } : {}).sort({ createdAt: -1 }),
...
router.put('/picture-folders/:id', authMiddleware, roleMiddleware(['admin']), async (req, res) => {
  try {
    const { id } = req.params
    const { 
      name, 
    ...
    if (isPurchasable !== undefined) updateData.isPurchasable = Boolean(isPurchasable)
    if (thumbnailUrl !== undefined) updateData.thumbnailUrl = thumbnailUrl
    if (previewImageUrl !== undefined) updateData.previewImageUrl = previewImageUrl
    
    const folder = await PictureFolder.findByIdAndUpdate(
      id, 
      updateData, 
      { new: true }
    )
...
router.delete('/picture-folders/:id', authMiddleware, roleMiddleware(['admin']), async (req, res) => {
  try {
    const { id } = req.params
    ...
    const children = await PictureFolder.find({ parentId: id })
    if (children.length > 0) {
      return res.status(400).json({ 
        message: `Cannot delete folder "${folder.name}" because it contains ${children.length} subfolder(s). Please delete the subfolders first.` 
      })
    }
    
    // Check if folder has templates
    const templates = await PictureTemplate.find({ folderId: id })
    if (templates.length > 0) {
      return res.status(400).json({ 
        message: `Cannot delete folder "${folder.name}" because it contains ${templates.length} template(s). Please delete the templates first.` 
      })
    }
...
router.put('/picture-templates/:id', authMiddleware, roleMiddleware(['admin']), 
  upload.fields([
    { name: 'previewImage', maxCount: 1 },
    { name: 'downloadImage', maxCount: 1 },
  ]),
    ...
    try {
      ...
      if (previewFile || downloadFile) {
        ...
        if (downloadFile) {
          ...
          updateData.format = downloadFile.originalname.split('.').pop().toLowerCase()
        }
      }
      
      
      const updatedTemplate = await PictureTemplate.findByIdAndUpdate(id, updateData, { new: true })
...
router.delete('/picture-templates/:id', authMiddleware, roleMiddleware(['admin']), async (req, res) => {
  try {
    const { id } = req.params
...
async function buildPicturePath(folderId) {
  ...
  while (current) {
    ...
    current = current.parentId ? await PictureFolder.findById(current.parentId) : null
  }
  return path
}
