...
const MyPictureFolders = () => {
  ...
  const [currentView, setCurrentView] = useState<"folders" | "templates">(
    "folders",
  );
      ...
      try {
        ...
        const purchasedFolderIds = new Set(
          purchases.flatMap((purchase: any) =>
            purchase.items
              .filter(
                (item: any) => item.type === "picture-folder" && item.folderId,
              )
  ...
  const handleDownloadAll = async (folder: PictureFolder) => {
    try {
      ...
      if (folderTemplates.length === 0) {
        toast.error("No pictures found in this folder");
        return;
      }

      toast.info(`Downloading ${folderTemplates.length} pictures...`);
      ...
      toast.error("Failed to download pictures. Please try again.");
    }
  };
        ...
        filteredFolders.length > 0 ? (
          ...
          >
            {filteredFolders.map((folder) => (
              <Card
                key={folder._id}
                className="group bg-white/80 backdrop-blur-lg border border-slate-200 hover:border-purple-300 hover:shadow-xl transition-all duration-300 overflow-hidden"
              >
                {viewMode === "grid" ? (
                  <>
                    <div
                      className="relative aspect-[4/3] bg-gradient-to-br from-purple-100 to-pink-100 flex items-center justify-center cursor-pointer overflow-hidden"
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-4">
                      <div
                        className="relative w-20 h-20 bg-gradient-to-br from-purple-100 to-pink-100 rounded-lg flex items-center justify-center cursor-pointer flex-shrink-0"
                        onClick={() => openFolder(folder)}
                      >
          ...
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg p-12 text-center border border-slate-200">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-3">
              No Picture Folders Yet
            </h3>
            <p className="text-slate-600 mb-6">
              You haven't purchased any picture folder collections yet. Start
