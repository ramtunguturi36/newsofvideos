  ...
  const getBreadcrumbs = () => {
    if (params.get('folderId')) {
      return [
        ...
        ...path.map(folder => ({ name: folder.name, path: `/user/dashboard?folderId=${folder._id}`, icon: FolderOpen })),
        { name: 'Current', path: '', icon: null }
      ];
    }
    
    const currentItem = navigationSections.flatMap(section => section.items).find(item => location.pathname === item.path);
    return [
      { name: 'Home', path: '/user/dashboard', icon: Home },
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-pink-50 text-slate-900 flex">
      ...
      <aside 
        className={cn(
          ...
          sidebarCollapsed ? 'md:w-16 md:translate-x-0' : 'md:w-80 md:translate-x-0',
          'md:static md:flex-shrink-0'
        )}
      >
        <div className="h-full flex flex-col">
          {/* Sidebar Header */}
          <div className={cn("border-b border-slate-200/50", sidebarCollapsed ? "p-4" : "p-8")}>
            <div className="flex items-center gap-3">
              ...
              {!sidebarCollapsed && (
                <div>
                  ...
                  <p className="text-sm text-slate-600">Professional Video Templates</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className={cn("border-b border-slate-200/50 bg-slate-50/30", sidebarCollapsed ? "p-4" : "p-6")}>
            <div className="flex items-center gap-4">
              ...
              {!sidebarCollapsed && (
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-slate-900 truncate">{user?.name || 'Marky'}</p>
                  <p className="text-sm text-slate-600 truncate">{user?.email || 'marky@example.com'}</p>
                </div>
              )}
            </div>
          </div>
                    ...
                    >
                      <div className="relative">
                        <item.icon className={cn(
                          ...
                          isActive ? 'text-purple-600' : 'text-slate-500 group-hover:text-purple-500'
                        )} />
                        {item.badge && (
          ...
          <div className={cn("border-t border-slate-200/50 mt-auto", sidebarCollapsed ? "p-2" : "p-6")}>
            <button
            ...
            >
              {isLoggingOut ? (
                <>
                  ...
                  {!sidebarCollapsed && <span className="ml-2">Signing out...</span>}
                </>
              ) : (
                <>
        ...
        {/* Main content */}
      <div className={cn("flex-1 flex flex-col overflow-hidden", sidebarCollapsed ? "md:ml-0" : "md:ml-0")}>
        {/* Top Bar - Redesigned */}
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-white/95 backdrop-blur-lg rounded-2xl shadow-xl py-2 z-50 border border-slate-200/50">
                      <div className="px-4 py-3 border-b border-slate-200/50">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-slate-900">{user?.name || 'Marky'}</p>
                            <p className="text-xs text-slate-600 truncate">{user?.email || 'marky@example.com'}</p>
                          </div>
                        </div>
                      </div>
                      <Link
                ...
                <div className="bg-gradient-to-r from-purple-50/80 to-indigo-50/80 backdrop-blur-lg rounded-2xl shadow-lg p-8 border border-purple-200/30 relative overflow-hidden">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-slate-600 text-lg mb-6">Here's what's happening with your account today.</p>
                    ...
                    <div className="mt-6 md:mt-0 md:ml-8">
                      <div className="bg-white/90 backdrop-blur-lg rounded-2xl p-6 border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-300">
                        <div className="text-center">
                          <div className="text-3xl font-bold text-slate-900 mb-1">{purchasedTemplates.size}</div>
                          <div className="text-sm text-slate-600 font-medium">Templates Owned</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-8 bg-white rounded-2xl shadow-lg p-8 border border-slate-200/50">
                    <div className="flex items-center justify-between mb-8">
                      <div>
                        ...
                        <p className="text-slate-600 text-lg">View and explore template types</p>
                      </div>
                      <div className="flex items-center space-x-4">
                        ...
                        <Button
                          onClick={() => setShowAllVideoFolders(!showAllVideoFolders)}
                          variant="outline"
                  ...
                  <div className="mb-8 bg-white rounded-2xl shadow-lg p-8 border border-slate-200/50">
                    <div className="flex items-center justify-between mb-8">
                      <div>
                        ...
                        <p className="text-slate-600 text-lg">View and explore picture template types</p>
                      </div>
                      <div className="flex items-center space-x-4">
                        <span className="text-sm text-slate-700 bg-slate-100 px-4 py-2 rounded-full border border-slate-200">
                        ...
                        <Button
                          onClick={() => setShowAllPictureFolders(!showAllPictureFolders)}
                          variant="outline"
              ...
              <div className="space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h1 className="text-3xl font-bold text-slate-900 mb-2">
                          Categories / {path[path.length - 1]?.name || 'Unknown'}
                        </h1>
                        <p className="text-slate-600 text-lg">Explore video templates in this category</p>
                      </div>
                      <Button
                            ...
                            {template.videoUrl ? (
                              ...
                              <div className="w-full h-full bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
                                <div className="text-center">
                                  ...
                                  <span className="text-slate-500 text-sm font-medium">No preview</span>
                                </div>
                              </div>
                            )}
                ...
                {filteredFolders.length === 0 && filteredTemplates.length === 0 && (
                  <div className="bg-gradient-to-r from-slate-50 to-blue-50 backdrop-blur-lg rounded-2xl shadow-xl p-16 text-center border border-slate-200/50 animate-fade-in">
                    ...
                    <p className="text-slate-600 text-lg mb-6">This category doesn't have any content yet, or your search didn't match any items.</p>
                    <Button
